...
namespace WEB_SLAM {
  ...
  class System {
    ...
    public:
      ...
      // Initialize the SLAM system. It launches the Local Mapping and Loop
      // Closing threads.
      ...
      // frame. Input image: RGB (CV_8UC3) or grayscale (CV_8U). RGB is
      // converted to grayscale. Input depthmap: Float (CV_32F). Returns the
      // camera pose (empty if tracking fails).
      Sophus::SE3f
    ...
    private:
      ...
      // It also decides when to insert a new keyframe, create some new
      // MapPoints and performs relocalization if tracking fails.
      Tracking *mpTracker;
