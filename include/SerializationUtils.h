...
namespace WEB_SLAM {
  ...
  void
  serializeSophusSE3(Archive &ar, Sophus::SE3f &T, const unsigned int version) {
    Eigen::Vector4f quat;
  ...
  void serializeDiagonalMatrix(Archive &ar, Eigen::DiagonalMatrix<float, dim>
  &D, const unsigned int version)
  {
  ...
  template <class Archive>
  void
  serializeMatrix(Archive &ar, const cv::Mat &mat, const unsigned int version) {
    cv::Mat matAux = mat;
