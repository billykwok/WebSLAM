...
int main(int argc, char **argv) {
  ...
  // process frames.
  WEB_SLAM::System SLAM(
      argv[1], argv[2], WEB_SLAM::System::IMU_STEREO, true, 0, file_name);
  ...
  for (seq = 0; seq < num_seq; seq++) {
    ...
    vector<WEB_SLAM::IMU::Point> vImuMeas;
    proccIm = 0;
    ...
    for (int ni = 0; ni < nImages[seq]; ni++, proccIm++) {
      ...
      if (ni > 0) {
        ...
        while (vTimestampsImu[seq][first_imu[seq]] <= vTimestampsCam[seq][ni]) {
          // vImuMeas.push_back(WEB_SLAM::IMU::Point(vAcc[first_imu],vGyro[first_imu],vTimestampsImu[first_imu]));
          vImuMeas.push_back(
              WEB_SLAM::IMU::Point(vAcc[seq][first_imu[seq]].x,
