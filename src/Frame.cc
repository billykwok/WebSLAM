...
namespace WEB_SLAM {
  ...
  Frame::Frame(const cv::Mat &imLeft,
      ...
      : mpcpi(NULL), mpORBvocabulary(voc), mpORBextractorLeft(extractorLeft),
        ...
        mK(K.clone()), mK_(Converter::toMatrix3f(K)),
  ...
  void
  Frame::ExtractORB(int flag, const cv::Mat &im, const int x0, const int x1) {
    ...
    for (vector<vector<cv::DMatch>>::iterator it = matches.begin();
      ...
      if ((*it).size() >= 2 && (*it)[0].distance < (*it)[1].distance * 0.7) {
        ...
        float sigma1 =
              ...
              sigma2 = mvLevelSigma2[mvKeysRight[(*it)[0].trainIdx + monoRight]
                                         .octave];
        float depth =
  ...
  bool
  Frame::isInFrustumChecks(MapPoint *pMP, float viewingCosLimit, bool bRight) {
    // 3D in absolute coordinates
    Eigen::Vector3f P = pMP->GetWorldPos();
