...
namespace WEB_SLAM {
                ...
                while (nNumKFs < 3 && j < vpCurrentCovKFs.size()) {
                  ...
                  Sophus::SE3d mTjc =
                      (pKFj->GetPose() * mpCurrentKF->GetPoseInverse())
                          .cast<double>();
                  g2o::Sim3 gSjc(
                      mTjc.unit_quaternion(), mTjc.translation(), 1.0);
  ...
  void LoopClosing::MergeLocal() {
    ...
    while (pNewChild) {
      pNewChild->EraseChild(
          pNewParent); // We remove the relation between the old
  ...
  void LoopClosing::MergeLocal2() {
...

    {
      ...
      // mpAtlas->GetCurrentMap()->KeyFramesInMap()
      // << endl;
  ...
  void LoopClosing::ResetIfRequested() {
    ...
    if (mbResetRequested) {
      ...
      mLastLoopKFid =
          mpAtlas->GetLastInitKFid(); // TODO old variable, it is not
    ...
    // tree
    {
      ...
      if (!mbStopGBA) {
        ...
        while (!lpKFtoCheck.empty()) {
          ...
          pKF->mvKeys[i].pt, 2, cv::Scalar(0, 255, 0)); cv::putText(imLeft,
          strNumOBs, pKF->mvKeys[i].pt, CV_FONT_HERSHEY_DUPLEX, 1,
          cv::Scalar(255, 0, 0));
              }
        ...
        double timeUpdateMap = std::chrono::duration_cast<
                                   std::chrono::duration<double, std::milli>>(
