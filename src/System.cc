...
namespace WEB_SLAM {
  ...
  System::System(const string &strVocFile,
    ...
    mpLocalMapper =
        new LocalMapping(this,
                         ...
                         mSensor == IMU_MONOCULAR || mSensor == IMU_STEREO ||
                             mSensor == IMU_RGBD,
                         strSequence);
    mptLocalMapping = new thread(&WEB_SLAM::LocalMapping::Run, mpLocalMapper);
    ...
    mptLoopClosing = new thread(&WEB_SLAM::LoopClosing::Run, mpLoopCloser);
  ...
  void System::SaveTrajectoryTUM(const string &filename) {
    ...
    list<WEB_SLAM::KeyFrame *>::iterator lRit =
  ...
  endl; return;
      ...
      // Transform all keyframes so that the first keyframe is at the origin.
      // After a loop closure the first keyframe might not be at the origin.
      Sophus::SE3f Twb; // Can be word to cam0 or world to b dependingo on IMU
  or not. if (mSensor==IMU_MONOCULAR || mSensor==IMU_STEREO ||
  mSensor==IMU_RGBD) Twb = vpKFs[0]->GetImuPose_(); else Twb =
  ...
  optimized by BA and pose graph).
      // We need to get first the keyframe pose and then concatenate the
  ...
  mpTracker->mlRelativeFramePoses.size() << endl;
      //cout << "size mpTracker->mlFrameTimes: " <<
  mpTracker->mlFrameTimes.size()
  << endl;
      //cout << "size mpTracker->mlbLost: " << mpTracker->mlbLost.size() <<
  endl;
  ...
  get a suitable keyframe. if (!pKF) continue;
          ...
          Trw = Trw * pKF->GetPose()*Twb; // Tcp*Tpw*Twb0=Tcb0 where b0 is the
  new world reference
