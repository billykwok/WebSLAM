...
namespace WEB_SLAM {
  ...
  bool Tracking::ParseCamParamFile(cv::FileStorage &fSettings) {
    ...
    if (mSensor == System::RGBD || mSensor == System::IMU_RGBD) {
      ...
      if (!node.empty() && node.isReal()) {
        ...
        std::cerr << "*DepthMapFactor parameter doesn't exist or is not a real "
                     "number*"
                  << std::endl;
        b_miss_params = true;
      }
  ...
  bool Tracking::ParseORBParamFile(cv::FileStorage &fSettings) {
    ...
    if (!node.empty() && node.isInt()) {
      ...
      std::cerr
          << "*ORBextractor.nFeatures parameter doesn't exist or is not an "
             "integer*"
          << std::endl;
      b_miss_params = true;
    }
    ...
    if (!node.empty() && node.isReal()) {
      ...
      std::cerr
          << "*ORBextractor.scaleFactor parameter doesn't exist or is not "
             "a real number*"
          << std::endl;
      b_miss_params = true;
    }
    ...
    if (!node.empty() && node.isInt()) {
      ...
      std::cerr << "*ORBextractor.nLevels parameter doesn't exist or is not an "
                   "integer*"
                << std::endl;
      b_miss_params = true;
    }
    ...
    if (!node.empty() && node.isInt()) {
      ...
      std::cerr
          << "*ORBextractor.iniThFAST parameter doesn't exist or is not an "
             "integer*"
          << std::endl;
      b_miss_params = true;
    }
    ...
    if (!node.empty() && node.isInt()) {
      ...
      std::cerr
          << "*ORBextractor.minThFAST parameter doesn't exist or is not an "
             "integer*"
          << std::endl;
      b_miss_params = true;
    }
  ...
  void Tracking::Track() {
    ...
    if (mState == NOT_INITIALIZED) {
      ...
      if (!mbOnlyTracking) {
        ...
        // Localization Mode: Local Mapping is deactivated (TODO Not available
        // in inertial mode)
        if (mState == LOST) {
          if (mSensor == System::IMU_MONOCULAR ||
              mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) {
  ...
  void Tracking::UpdateFrameIMU(const float s,
    ...
    unsigned int index = mnFirstFrameId;
    list<WEB_SLAM::KeyFrame *>::iterator lRit = mlpReferences.begin();
