  ...
  namespace IMU {
    ...
    Preintegrated::Preintegrated(Preintegrated *pImuPre)
        : dT(pImuPre->dT), C(pImuPre->C), Info(pImuPre->Info),
          ...
          dR(pImuPre->dR), dV(pImuPre->dV), dP(pImuPre->dP), JRg(pImuPre->JRg),
          JVg(pImuPre->JVg), JVa(pImuPre->JVa), JPg(pImuPre->JPg),
          JPa(pImuPre->JPa), avgA(pImuPre->avgA), avgW(pImuPre->avgW),
    ...
    Eigen::Matrix3f Preintegrated::GetUpdatedDeltaRotation() {
      ...
      return NormalizeRotation(dR *
                               Sophus::SO3f::exp(JRg * db.head(3)).matrix());
    }
